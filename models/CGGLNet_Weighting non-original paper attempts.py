...
'''
def process_tensor(input,th_l,th_h):

    # Find the index of the maximum value in the channel dimension
    max_indices_channel = input.argmax(dim=1, keepdim=True)

    # Create a tensor output of the same shape as the input and initialize it to th_l
    output = torch.full_like(input, th_l)

    # Set the position of the maximum value to th_h
    indices_expand = max_indices_channel.expand(-1, input.size(1), -1, -1)
    output.scatter_(1, indices_expand, th_h)

    return output
'''


def process_tensor(input_tensor, th_l, th_h, diff_th):

    # Find the indexes of the largest and second highest values in the channel dimension
    max_indices = input_tensor.argmax(dim=1, keepdim=True)
    max_values = torch.gather(input_tensor, 1, max_indices)

    # Get the index of the second highest value and calculate its difference from the highest value
    input_tensor.scatter_(1, max_indices, 0.0)  # Exclude the location of the maximum value
    second_max_values, _ = input_tensor.max(dim=1, keepdim=True)
    second_indices = input_tensor.argmax(dim=1, keepdim=True)
    diff = max_values - second_max_values

    # Create a tensor output with the same shape as input_tensor and initialize it to th_l
    output_tensor = torch.full_like(input_tensor, th_l)

    # Determine if the second highest channel value is within diff_th of the highest value
    mask = (diff <= diff_th).float()

    # Set the position of the second highest value to a random number from th_l to th_h
    random_values = torch.rand_like(output_tensor) * th_h + th_l
    random_values = random_values * mask  # Add only if conditions are met
    output_tensor.scatter_add_(1, second_indices, random_values)

    # Set the position of the maximum value to th_h
    output_tensor.scatter_(1, max_indices, th_h)
